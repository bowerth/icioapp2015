

about.output <- list(
  ## h3("ICIO Indicator 2015")
  ## ,
  p("Version 1.0 (01/07/2015)")
  ,
  p("Author:", a(href = "http://www.oecd.org/sti/", "OECD STI"))
  ,
  p("Contact:", a(href = "mailto:stan.contact@oecd.org?subject=ICIO%20Indicators", "stan.contact@oecd.org"))
  ,
  p("Source code and documentation on",
    a(href = "https://github.com/bowerth/icioapp2015", "GitHub"))
  ,
  p("Built using", a(href = "http://www.rstudio.com/shiny/", "shiny"),
    "and", a(href = "http://rstudio.github.io/shinydashboard/", "shinydashboard"),
    "by", a(href = "http://www.rstudio.com/", "RStudio"))
  )

